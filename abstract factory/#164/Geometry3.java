...
class FlatCircle implements Circle {
    ...
    public void draw() { System.out.println("Drawing a flat, minimalist Circle."); }
}

class FlatRectangle implements Rectangle {
    double length = 10, width = 5;
    public double area() { return length * width; }
    public void draw() { System.out.println("Drawing a flat, minimalist Rectangle."); }
}

class FlatSquare extends FlatRectangle implements Square {
    double side = 5;
    public FlatSquare() { this.length = side; this.width = side; }
    public void draw() { System.out.println("Drawing a flat, minimalist Square."); }
}
...
class ThreeDCircle implements Circle {
    ...
    public void draw() { System.out.println("Drawing a glossy, 3D Shaded Circle."); }
}

class ThreeDRectangle implements Rectangle {
    double length = 10, width = 5, height = 2;
    public double area() { return 2 * (length * width + length * height + width * height); }
    public void draw() { System.out.println("Drawing a glossy, 3D Shaded Rectangle."); }
}

class ThreeDSquare extends ThreeDRectangle implements Square {
    double side = 5;
    public ThreeDSquare() { this.length = side; this.width = side; this.height = side; }
    public void draw() { System.out.println("Drawing a glossy, 3D Shaded Square."); }
}
...
class FlatThemeFactory implements ThemeFactory {
    public Circle createCircle() { return new FlatCircle(); }
    public Rectangle createRectangle() { return new FlatRectangle(); }
    public Square createSquare() { return new FlatSquare(); }
}

class ThreeDThemeFactory implements ThemeFactory {
    public Circle createCircle() { return new ThreeDCircle(); }
    public Rectangle createRectangle() { return new ThreeDRectangle(); }
    public Square createSquare() { return new ThreeDSquare(); }
}
...
public class Geometry3 {
    public static void main(String args[]) {
        ...
        switch (themeChoice) {
            case 1:  factory = new FlatThemeFactory(); break;
            case 2:  factory = new ThreeDThemeFactory(); break;
            default: factory = new FlatThemeFactory();
        }
